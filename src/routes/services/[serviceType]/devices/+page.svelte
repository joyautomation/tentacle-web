  ...
  const devices = $derived(() => {
    ...
    for (const [deviceId, dvars] of deviceMap) {
        ...
        if (dotIdx === -1) {
          ...
          if (v.datatype !== 'udt') {
            ...
            if (!tagMap.has(v.variableId)) {
              tagMap.set(v.variableId, { name: v.variableId, udtType: v.udtType ?? undefined, members: [] });
            } else {
          ...
          if (!tagMap.has(baseName)) {
            tagMap.set(baseName, { name: baseName, udtType: v.udtType ?? undefined, members: [] });
          }
      ...
      const scalarTags = scalars
        .map(s => ({ name: s.variableId, variable: s, members: [] }) as TagNode)
        .sort((a, b) => a.name.localeCompare(b.name));

      result.push({ deviceId, typeGroups, scalars: scalarTags, totalVars: dvars.length });
    }
      ...
      {#each devices() as device}
        <div class="tree-node">
          <button class="tree-toggle" onclick={() => toggleDevice(device.deviceId)}>
            <svg class="chevron" class:expanded={expandedDevices[device.deviceId]} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M9 18l6-6-6-6"/>
            </svg>
            <span class="device-icon">DEV</span>
            <span class="tree-label">{device.deviceId}</span>
            {#if data.deviceInfo[device.deviceId]}
              <span class="device-host">{data.deviceInfo[device.deviceId].host}:{data.deviceInfo[device.deviceId].port}</span>
            {/if}
              ...
              {#each device.typeGroups as group}
                <div class="tree-node">
                  <button class="tree-toggle" onclick={() => toggleTag(device.deviceId + ':type:' + group.typeName)}>
                    <svg class="chevron" class:expanded={expandedTags[device.deviceId + ':type:' + group.typeName]} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                      <path d="M9 18l6-6-6-6"/>
                    </svg>
                      ...
                      {#each group.instances as tag}
                        <div class="tree-node">
                          <button class="tree-toggle" onclick={() => toggleTag(device.deviceId + '.' + tag.name)}>
                            <svg class="chevron" class:expanded={expandedTags[device.deviceId + '.' + tag.name]} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                              <path d="M9 18l6-6-6-6"/>
                            </svg>
